import { PencilSquareIcon, PlusCircleIcon, TrashIcon } from "@heroicons/react/24/solid";
import { useEffect, useState } from "react";
import { RiSearchLine } from "react-icons/ri";
import { Alert } from "../../../components/Alert";
import { Header } from "../../../components/Header";
import { ModalDelete } from "../../../components/Modal/Delete";
import { ModalUpInsertResolution } from "../../../components/Modal/UpInsertResolution";
import { Navbar } from "../../../components/Navbar";
import { useResolution } from "../../../hooks/useAPI";
import { INotify } from "../../../interfaces/INotify";
import { IResolution } from "../../../interfaces/IProduct";

export const Resolution = () => {

    const [resolutionData, setResolutionData] = useState<IResolution[] | null>(null);
    const [resolution, setResolution] = useState<IResolution | null>(null);

    const [notify, setNotify] = useState<INotify>();

    const [modalDelete, setModalDelete] = useState<Boolean>();
    const [modalUpInsert, setModalUpInsert] = useState<Boolean>();

    useEffect(() => {
        getResolution();
    }, []);

    const getResolution = async () => {
        try {
            const data = await useResolution.GetAllSResolution();
            setResolutionData(data.resolution);
        } catch (error) {
            console.log(error);
        }
    };

    const searchResolution = async (search: string) => {
        try {
            const request = await useResolution.GetResolution(search);
            setResolutionData(request.resolution);
        } catch (error) {
            console.log(error);
        }
    };

    const createResolution = async (e: IResolution) => {
        try {

            const request = await useResolution.CreateResolution(e);

            if (!request.resolution.id) {
                SendNotification({ message: 'There was an error creating!', type: 'ERROR', status: true });
                setModalUpInsert(false);
                return;
            }

            SendNotification({ message: 'Resolution was successfully creating!', type: 'SUCCESS', status: true });
            setModalUpInsert(false);
            getResolution();

        } catch (error) {
            console.log(error);
            SendNotification({ message: 'There was an error creating!', type: 'ERROR', status: true });
            setModalUpInsert(false);
        }
    };

    const updateResolution = async (e: IResolution) => {
        try {

            const request = await useResolution.UpdateResolution(e);

            if (!request.resolution.id) {
                SendNotification({ message: 'There was an error update!', type: 'ERROR', status: true });
                setModalUpInsert(false);
                return;
            }

            SendNotification({ message: 'Resolution successfully update!', type: 'SUCCESS', status: true });
            setModalUpInsert(false);
            getResolution();

        } catch (error) {
            console.log(error);
            SendNotification({ message: 'There was an error update!', type: 'ERROR', status: true });
            setModalUpInsert(false);
        }
    };

    const deleteResolution = async () => {
        try {

            if (!resolution?.id) {
                SendNotification({ message: 'ID not found', type: 'ERROR', status: true });
                setModalDelete(false);
                return;
            }

            const request = await useResolution.DeleteResolution(resolution.id.toString());

            if (!request.resolution.id) {
                SendNotification({ message: 'There was an error deleting!', type: 'ERROR', status: true });
                setModalDelete(false);
                return;
            }

            SendNotification({ message: 'Resolution was successfully deleted!', type: 'SUCCESS', status: true });
            setModalDelete(false);
            getResolution();

        } catch (error) {
            console.log(error);
            SendNotification({ message: 'There was an error deleting!', type: 'ERROR', status: true });
            setModalDelete(false);
        }
    };

    const onSubmit = async (e: any) => {

        if (!resolution?.id) {
            createResolution(e);
            return;
        }
        updateResolution(e);
    };

    const SendNotification = async (notify: INotify) => {
        setNotify({ message: notify.message, status: notify.status, type: notify.type });
        setTimeout(() => setNotify({ message: notify.message, status: false, type: notify.type }), 3500);
    }

    return (
        <>
            <Header />
            <div className='flex'>
                <nav className='w-[250px]'>
                    <Navbar />
                </nav>
                <main className="w-[calc(100%-250px)]">
                    <div className="w-full h-[calc(100vh-60px)] mx-auto px-[2rem] py-6">
                        <div className={`w-full fixed top-[4rem] flex justify-end pr-4`}>
                            <Alert type={notify?.type} message={notify?.message} status={notify?.status} />
                        </div>
                        <div className="w-full h-5rem flex justify-between items-center">
                            <div className="text-[#bebebe]">
                                <h1 className="font-medium text-[1.5rem]">Resolution</h1>
                                <h2>This page is used to do all the resolution configuration part of the product.</h2>
                            </div>
                            <div className="flex gap-6 items-center">
                                <div className="bg-zinc-800 flex gap-2 items-center rounded-2xl px-2">
                                    <RiSearchLine className="text-[#bebebe]" />
                                    <input
                                        onChange={(e) => searchResolution(e.target.value)}
                                        type="text"
                                        className="w-[300px] bg-transparent outline-none px-1 py-[.3rem] rounded-2xl text-[#bebebe]"
                                        placeholder="pesquisar" />
                                </div>
                                <PlusCircleIcon
                                    className="w-10 text-[#3B82F6] hover:scale-110 cursor-pointer"
                                    onClick={() => { setResolution(null); setModalUpInsert(!modalUpInsert) }} />
                            </div>
                        </div>
                        <section className="relative w-full h-[calc(90%-5rem)] mt-[2rem] py-2 overflow-auto">
                            <div className="overflow-x-auto relative">
                                {resolutionData && resolutionData.length > 0
                                    ?
                                    <table className="min-w-[1366px] w-full text-sm text-left text-[#bebebe]">
                                        <thead className="text-xs text-[#3B82F6] uppercase ">
                                            <tr className="">
                                                <th className="py-3">#</th>
                                                <th className="py-3">Model</th>
                                                <th className="py-3">systemVersion</th>
                                                <th className="py-3">equalResolution</th>
                                                <th className="py-3">VerticalResolution</th>
                                                <th className="py-3">HorizontalResolution</th>
                                                <th className="py-3">Status</th>
                                                <th className="py-3">Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody className="">
                                            {resolutionData.map((item: IResolution, index: number) => (
                                                <tr className="" key={index}>
                                                    <td className="py-4">{item.id}</td>
                                                    <td className="py-4">{item.modelo}</td>
                                                    <td className="py-4">{item.systemVersion}</td>
                                                    <td className="py-4">
                                                        {item.equalResolution
                                                            ?
                                                            <span className="w-14 flex justify-center border border-[#00e170] text-[#00e170] py-[.1rem] rounded-lg text-[.7rem] font-bold">ON</span>
                                                            :
                                                            <span className="w-14 flex justify-center border border-[#db021f] text-[#db021f] py-[.1rem] rounded-lg text-[.7rem] font-bold">OFF</span>
                                                        }
                                                    </td>
                                                    <td className="py-4">{item.currentVerticalResolution}</td>
                                                    <td className="py-4">{item.currentHorizontalResolution}</td>
                                                    <td className="py-4">
                                                        {item.status
                                                            ?
                                                            <span className="w-14 flex justify-center border border-[#00e170] text-[#00e170] py-[.1rem] rounded-lg text-[.7rem] font-bold">ENABLE</span>
                                                            :
                                                            <span className="w-14 flex justify-center border border-[#db021f] text-[#db021f] py-[.1rem] rounded-lg text-[.7rem] font-bold">DISABLE</span>
                                                        }
                                                    </td>
                                                    <td className="w-full py-4 px-1 flex justify-between">
                                                        <PencilSquareIcon
                                                            className="w-5 hover:scale-110 cursor-pointer"
                                                            onClick={() => { setResolution(item); setModalUpInsert(!modalUpInsert) }} />
                                                        <TrashIcon
                                                            className="w-5 hover:scale-110 cursor-pointer text-[#db021f]"
                                                            onClick={() => { setResolution(item); setModalDelete(!modalDelete) }} />
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                    :
                                    <div>
                                        <span className="text-white">D000</span>
                                    </div>
                                }
                            </div>
                        </section >
                        {modalDelete &&
                            <ModalDelete
                                isOpen={() => setModalDelete(!modalDelete)}
                                execute={deleteResolution}
                                model={resolution}
                                title="Resolution" />
                        }
                        {
                            modalUpInsert &&
                            <ModalUpInsertResolution
                                isOpen={() => setModalUpInsert(!modalUpInsert)}
                                execute={(e) => onSubmit(e)}
                                resolution={resolution} />
                        }
                    </div >
                </main>
            </div>
        </>
    );
};